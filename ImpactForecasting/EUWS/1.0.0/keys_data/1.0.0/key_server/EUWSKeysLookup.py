...
class EUWSKeysLookup(Lookup):
    ...
    def process_locations(self, df_ptf_orig):
        ...
        df_location_fields['PrecisionName'] = df_location_fields['PrecisionName'].str.upper().str.replace(' ', '')
    ...
    def assign_vulnerabilityID(self, df_in):
        merging_cols = ['OCCUPANCYSCHEME', 'OCCUPANCYCLASS', 'COVERAGE', 'COUNTRYISO', 'REGION']
        df_in['REGION'] = df_in['REGION'].fillna(0).astype('int')
        df_in['OCCUPANCYCLASS'] = df_in['OCCUPANCYCLASS'].astype('int')
        df_in['OCCUPANCYSCHEME'] = df_in['OCCUPANCYSCHEME'].str.lower()
        df_vuln = pd.read_parquet(self.VULNERABILITY_LOOKUP_FILE)
        df_vuln.columns = df_vuln.columns.str.upper()
